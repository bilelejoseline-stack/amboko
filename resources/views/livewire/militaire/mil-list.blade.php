...
<div class="p-4 dark:bg-gray-900 dark:text-gray-100 min-h-screen">
    ...
    <div class="p-6 mt-20 bg-gray-900 min-h-screen text-white">

      @if (session()->has('message'))
          <div class="bg-green-600 dark:bg-green-800 text-white dark:text-white p-3 rounded mb-4">
              {{ session('message') }}
          </div>
            ...
            <table class="w-full table-auto text-white">
                <thead>
                    <tr class="bg-gray-700 text-gray-200 uppercase text-sm leading-normal">
                        <th class="py-2 px-6 text-left">Avatar</th>
                        <th class="py-2 px-6 text-left">Matricule</th>
                        <th class="py-2 px-6 text-left">Nom Complet</th>
                        <th class="py-2 px-6 text-left">Grade</th>
                        <th class="py-2 px-6 text-left">Statut</th>
                ...
                <tbody class="text-gray-300 text-sm">
                    ...
                    @forelse($militaires as $militaire)
                        <tr class="border-b border-gray-700 hover:bg-gray-600">
                            <td class="py-1 px-6 text-left"><img src="{{ $militaire->avatar }}" alt="Avatar" class="h-6 w-6 rounded-full mx-auto" /></td>
                            <td class="py-1 px-6 text-left">{{ $militaire->matricule }}</td>
                            <td class="py-1 px-6 text-left">{{ $militaire->nom_complet }}</td>
                            <td class="py-1 px-6 text-left">{{ $militaire->grade->nom_grade ?? 'N/A' }}</td>
                            <td class="py-1 px-6 text-left">{{ $militaire->statut }}</td>
                            <td class="py-1 px-6 text-center">
                                <div class="flex item-center justify-center" x-data="{ confirmDelete: false }">
                                    <a href="{{ route('militaire.show', $militaire->slug) }}" class="w-4 mr-2 transform hover:text-blue-400 hover:scale-110">
                                    ...
                                    <a href="{{ route('militaire.edit', $militaire->slug) }}" class="w-4 mr-2 transform hover:text-blue-400 hover:scale-110">
                                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                          stroke="currentColor" class="w-5 h-5 mr-2">
                                          <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <p class="text-gray-700 dark:text-gray-300 mb-6 leading-relaxed text-center">
                                                ...
                                                <span class="font-bold text-red-600 dark:text-red-400">{{ $militaire->grade->nom_grade ?? 'N/A' }} {{ $militaire->nom_complet }}</span> ?
